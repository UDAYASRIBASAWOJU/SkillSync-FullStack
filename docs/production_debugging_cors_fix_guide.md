...
- Requests appear to go to `https://skillsync.udayasri.dev/api/*` instead of `https://api.skillsync.udayasri.dev/api/*`

### Root Cause
The frontend is misconfigured to point to the **Vercel frontend domain** (`skillsync.udayasri.dev`) for API calls instead of the **EC2 backend API Gateway** (`api.skillsync.udayasri.dev`). This occurs when:
...
Domain Routing:
• Frontend UI:   skillsync.udayasri.dev       → Vercel Static Hosting
• API Gateway:   api.skillsync.udayasri.dev   → EC2 Load Balancer/Ingress
• EC2 Direct:    35.153.59.2:80 (internal) → API Gateway (for debugging)
```
...
3. Add a new variable:
   - **Name**: `VITE_API_URL`
   - **Value**: `https://api.skillsync.udayasri.dev`
...
nslookup skillsync.udayasri.dev

# API Gateway (should resolve to EC2 Elastic IP)
nslookup api.skillsync.udayasri.dev
```
...
```bash
curl -X OPTIONS https://api.skillsync.udayasri.dev/api/auth/initiate-registration \
...
**Property**: `app.cors.allowed-origins=${ALLOWED_ORIGINS:https://skillsync.udayasri.dev}`
...
curl http://35.153.59.2:8080/actuator/health/readiness
...
**Fix**:
1. Update .env file: `ALLOWED_ORIGINS=https://skillsync.udayasri.dev,https://localhost:5173`
2. Restart services: `docker-compose down && docker-compose up -d`

### Issue 3: "Requests to skillsync.udayasri.dev returning Vercel 404"
...
- [ ] DNS records point to correct hosts:
  - `skillsync.udayasri.dev` → Vercel
  - `api.skillsync.udayasri.dev` → EC2 (35.153.59.2)
...
- [ ] Registration endpoint returns data (not 502): 
  ```bash
  curl -X POST https://api.skillsync.udayasri.dev/api/auth/initiate-registration \
    -H "Content-Type: application/json" \
    -H "Origin: https://skillsync.udayasri.dev" \
    -d '{"email":"test@gmail.com"}'
  ```
...
- **Eureka Dashboard**: http://localhost:8761 (dev) or https://api.skillsync.udayasri.dev/eureka-ui/ (prod)
...
graph TD
    ...
    B -->|Prod| D[Check api.skillsync.udayasri.dev DNS]
