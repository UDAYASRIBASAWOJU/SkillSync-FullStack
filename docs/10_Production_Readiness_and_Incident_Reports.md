...
```bash
curl -i https://skillsync.udayasri.dev/auth/actuator/health
```
...
- Added Spring Cloud Gateway global CORS:
  - Allowed origins include `https://skillsync.udayasri.dev`
...
- Default allowed origins now include `https://skillsync.udayasri.dev`.
...
```bash
ALLOWED_ORIGINS=https://skillsync.udayasri.dev,https://skill-sync-sage.vercel.app,http://localhost:3000,http://localhost:5173
APP_BASE_URL=https://skillsync.udayasri.dev
```
...
2. If Authorization Code flow is enabled later, add redirect URI exactly:
   - `https://skillsync.udayasri.dev/auth/oauth2/code/google`
...
```bash
curl -i https://skillsync.udayasri.dev/auth/actuator/health
curl -i https://skillsync.udayasri.dev/api/auth/login
```
...
```bash
curl -i -X OPTIONS https://skillsync.udayasri.dev/api/auth/login \
  -H "Origin: https://skillsync.udayasri.dev" \
...
- `Access-Control-Allow-Origin: https://skillsync.udayasri.dev`
...
- `https://skillsync.udayasri.dev/swagger-ui.html`
...
- **Base URL Awareness:** Replaced all hardcoded `http://localhost` usages in the notification microservice with `https://skillsync.udayasri.dev` configuration targets. 
...
- **URL Overrides:** Swapped default endpoint structures favoring `https://api.skillsync.udayasri.dev` inside Axios base configuration.
...
```mermaid
graph LR
  FE["Frontend<br>skillsync.udayasri.dev<br>(Vercel)"] -->|HTTPS + Cookies| GW["API Gateway<br>api.skillsync.udayasri.dev<br>(EC2, port 80â†’8080)"]
...
> [!IMPORTANT]
> **NGINX has been removed** from the architecture. The API Gateway is the sole ingress point. The `Backend/nginx/` directory is empty. SSL termination for `api.skillsync.udayasri.dev` is handled externally (Cloudflare proxy or Certbot on EC2 host â€” **not within Docker Compose**).
...
|---|-----------|--------|----------|
| 1 | **Cookie attributes** | âœ… Correct | `HttpOnly=true`, `Secure=true`, `SameSite=None`, `Domain=.udayasri.dev`, `Path=/`, `MaxAge` set â€” [AuthController.java:116-126](file:///f:/SkillSync/Backend/auth-service/src/main/java/com/skillsync/auth/controller/AuthController.java#L116-L126) |
...
| 5 | **Frontend axios client** | âœ… Correct | `withCredentials: true`, base URL = `https://api.skillsync.udayasri.dev`, Bearer token in header â€” [axios.ts](file:///f:/SkillSync/Frontend/src/services/axios.ts) |
...
curl -X PUT "https://api.skillsync.udayasri.dev/api/auth/users/1/role?role=ROLE_ADMIN" \
...
**Exploit:**
```bash
curl https://api.skillsync.udayasri.dev/api/auth/internal/users/1
...
<form action="https://api.skillsync.udayasri.dev/api/auth/users/1/role?role=ROLE_ADMIN" method="POST">
...
response.addHeader("Set-Cookie", ResponseCookie.from("XSRF-TOKEN", csrfToken)
    .domain(".udayasri.dev").path("/").secure(true).httpOnly(false)
...
curl -X POST https://api.skillsync.udayasri.dev/api/auth/setup-password \
...
```nginx
# /etc/nginx/sites-available/api.skillsync.udayasri.dev
server {
    listen 80;
    server_name api.skillsync.udayasri.dev;
...
server {
    ...
    ssl_certificate /etc/letsencrypt/live/api.skillsync.udayasri.dev/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/api.skillsync.udayasri.dev/privkey.pem;
...
> [!WARNING]
> **Severity: HIGH (availability)** â€” `https://api.skillsync.udayasri.dev/health` returns HTTP 502.
...
- name: Verify deployment
  run: |
    for i in {1..30}; do
      if curl -fs https://api.skillsync.udayasri.dev/health; then
