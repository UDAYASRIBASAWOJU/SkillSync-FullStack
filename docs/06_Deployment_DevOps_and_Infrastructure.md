...
|-----------|----------|-----|-------|
| Frontend | Vercel | `https://skillsync.udayasri.dev` | React SPA (Vite + TailwindCSS) |
| Backend API | AWS EC2 | `https://api.skillsync.udayasri.dev` | Docker Compose, 9 services |
| DNS | Cloudflare | `udayasri.dev` zone | Proxy mode (orange cloud) |
...
```
skillsync.udayasri.dev      â†’ Vercel (CNAME to cname.vercel-dns.com)
api.skillsync.udayasri.dev  â†’ EC2 Public IP (A record, Cloudflare proxied)
```
...
|----------|----------|---------|-------|
| `https://api.skillsync.udayasri.dev/health` | `{"status":"UP"}` | 5s | Application health |
| `https://api.skillsync.udayasri.dev/actuator/health` | `{"status":"UP"}` | 10s | Spring Boot health |
...
Client (skillsync.udayasri.dev) -> API Gateway (api.skillsync.udayasri.dev, port 80 -> container 8080) -> Microservices
...
```mermaid
graph TD
  FE[Frontend: https://skillsync.udayasri.dev] --> GW[Gateway API: https://api.skillsync.udayasri.dev\nEC2 host port 80 -> container 8080]
...
- Default allowed origin set to `https://skillsync.udayasri.dev`.
- Credentials enabled.
...
- OpenAPI `servers` now use `${APP_PUBLIC_BASE_URL:https://api.skillsync.udayasri.dev}`.
...
- Axios default base URL set to `https://api.skillsync.udayasri.dev` via `VITE_API_URL` fallback.
...
curl -i https://api.skillsync.udayasri.dev/actuator/health

# Auth path compatibility
curl -i https://api.skillsync.udayasri.dev/auth/actuator/health

# API path
curl -i -X POST https://api.skillsync.udayasri.dev/auth/register

# Swagger config
curl -i https://api.skillsync.udayasri.dev/v3/api-docs/swagger-config
```

Manual tests:

- Swagger UI at `https://api.skillsync.udayasri.dev/swagger-ui.html` executes API requests successfully.
